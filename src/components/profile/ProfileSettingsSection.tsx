...
export default function ProfileSettingsSection() {
  ...
  const handleLanguageChange = async (value: string) => {
    ...
    const languageLabel = langs.find(lang => lang.value === value)?.label ?? value
    try {
      await setLang(value)
      toast.success(t`已切换为 ${languageLabel}`)
    } catch {
      toast.error(t('语言切换失败，请重试'))
    }
  }
    ...
    <section className="flex flex-col gap-4">
      ...
      <div className="border border-[var(--carbon-border)] bg-[var(--carbon-surface)] p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex shrink-0 items-center rounded-md border border-[var(--carbon-border)] bg-[var(--carbon-surface-subtle)] p-0.5">
        ...
        <div className="mt-4 flex items-center justify-between gap-3 border-t border-[var(--carbon-border)] pt-4">
          <div className="flex min-w-0 flex-1 items-center gap-4">
          ...
          <div className="flex shrink-0 items-center rounded-md border border-[var(--carbon-border)] bg-[var(--carbon-surface-subtle)] p-0.5">
            {(['kg', 'jin'] as const).map(value => (
              <button
                ...
                className={`flex h-7 items-center justify-center rounded px-3 text-xs font-medium whitespace-nowrap transition-colors ${
                  unit === value
                    ? 'bg-[var(--carbon-surface)] text-[var(--carbon-primary)] shadow-sm'
                    : 'text-[var(--carbon-text-secondary)] hover:text-[var(--carbon-text)]'
                }`}
        ...
        <div className="mt-4 flex items-center justify-between gap-3 border-t border-[var(--carbon-border)] pt-4">
          ...
          <select
            ...
            className="h-8 shrink-0 border border-[var(--carbon-border)] bg-[var(--carbon-surface-subtle)] px-2 text-xs text-[var(--carbon-text)] outline-none focus:border-[var(--carbon-primary)]"
          >
            {langs.map(lang => (
