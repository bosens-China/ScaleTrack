...
import { getWeekStart, groupRecordsByWeek } from '@/utils/week'
import {
  formatWeight,
  formatWeightValue,
  fromDisplayWeight,
...
export default function TrendsRecordList({ records, goal, onUpdateRecord, onDeleteRecord }: Props) {
  ...
  const recordGroups = groupRecordsByWeek(recentRecords, record => record.date, currentLang)
  const currentWeekKey = getWeekStart(dayjs(), currentLang).format('YYYY-MM-DD')
  const previousRecordById = new Map(
    recentRecords.map((record, index) => [record.id, recentRecords[index + 1]]),
  )
  ...
  return (
    <section className="mt-6 border border-[var(--carbon-border)] bg-[var(--carbon-surface)]">
      ...
      <div className="trends-record-list carbon-scrollbar flex flex-col gap-4 p-3">
        {recordGroups.map(group => (
          <section key={group.key} className="flex flex-col border border-[var(--carbon-border)]">
            <div className="flex items-center gap-3 bg-[var(--carbon-surface-subtle)] px-3 py-2">
              <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--carbon-text-secondary)]">
                {group.key === currentWeekKey ? t('本周') : t('自然周')}
              </span>
              <span className="h-px flex-1 bg-[var(--carbon-border)]" />
              <span className="text-[10px] font-medium text-[var(--carbon-text-secondary)]">
                {formatAppDate(group.startDate, 'monthDay')} —{' '}
                {formatAppDate(group.endDate, 'monthDay')}
              </span>
            </div>
            {group.items.map(record => {
              const prevRecord = previousRecordById.get(record.id)
